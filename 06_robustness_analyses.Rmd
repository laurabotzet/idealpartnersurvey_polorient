...
###### Standardized Coefficients
```{r}
standardize_parameters(model_pref_politicalsim_robustcheck, method = "refit", ci = 0.997)
#ci(standardize_parameters(model_pref_politicalsim_robustcheck, method = "refit"), ci = 0.997)
```

###### Plot {.active}
```{r}
effects1 = lmer(pref_politicalsim ~ political_orientation + age + (1|country),
             data = data_included_documented) %>%
...
effects2 = lmer(pref_politicalsim ~ political_orientation + age + I(political_orientation^2) +
                  (1 + political_orientation | country),
             data = data_included_documented) %>%
  allEffects(confidence.level = 0.997, xlevels = 49) %>%
  as.data.frame()
