...
### Language
```{r}
n = length(data_lim_included_documented)

table(data_lim_included_documented$language)

round((table(data_lim_included_documented$language)/n)*100,2)
```
@Laura Was genau hast du hier berechnet? Die prozentualen Anteile wie viele Fragebögen in welchen Sprachen ausgefüllt wurden? Evtl. macht es hier für mich auch gerade wenig Sinn wegen des limitierten Datensatzes.
...
```
@Laura: For the descriptives including ideal_age? or from here on only ideal_age_rel?
@Problem @Laura R will not create a correlation table. Error message:
Fehler in as.data.frame.default(data) : 
