---
title: <font color="#66C2A5">Exploratory Analyses Continents</font>
...
data_included_documented <- data_included_documented %>%
  mutate(continent1 = countrycode(country,
                                 origin = "country.name",
                                 destination = "region"),
         continent1 = ifelse(country == "Micronesia",
                            "East Asia & Pacific",
                            continent1),
         continent2 = countrycode(country,
                                 origin = "country.name",
                                 destination = "continent"),
         continent2 = ifelse(country == "Micronesia",
                            "Oceania",
                            continent2))

table(data_included_documented$continent1)
table(data_included_documented$continent2)

data_included_documented = data_included_documented %>%
  mutate(continent_final = ifelse(continent2 == "Africa", "Africa",
                                  ifelse(continent2 == "Asia", "Asia",
                                         ifelse(continent2 == "Europe", "Europe",
                           ifelse(continent2 == "Oceania", "Oceania",
                                  ifelse(continent1 == "Latin America & Caribbean",
                                         "Latin & South America",
                                         ifelse(continent1 == "North America",
                                                "North America", NA)))))))

data_included_documented <- data_included_documented %>%
  mutate(continent_final = ifelse(country %in% c("Afghanistan", "Bahrain", "Iran",
                                                 "Iraq", "Israel", "Jordan", 
                                  ...
                                  "Middle East & Central Asia", continent_final),
         continent_final = ifelse(country %in% c("Armenia", "Burma",
                                                 "China",
                                  ...
                                  "South & East Asia", continent_final))

x <- data_included_documented %>% 
  select(country, continent_final) %>%
  group_by(country, continent_final) %>%
  summarize(n = n()) %>%
  arrange(continent_final, country)

write.csv2(x, file = "country_continents.csv")

continents <- x %>%
  group_by(continent_final) %>%
...
```{r}
regions = continents %>% filter(participants > 500)
data_included_documented_reg = data_included_documented %>%
  filter(continent_final %in% regions$continent_final)
```

```{r}
continents_reg =
  data_included_documented_reg %>%
  select(continent_final, country) %>%
  table() %>%
  as.data.frame() %>%
  filter(Freq != 0) %>%
  arrange(-Freq)
...
data_included_documented_reg_wide = data_included_documented_reg %>%
  pivot_wider(names_from = continent_final, values_from = political_orientation) %>%
...
data_graph$name = factor(rep(
  c(
    paste0(
      "Europe (n = ",
      sum(data_included_documented_reg$continent_final == "Europe"),
      ")"
    ),
    paste0(
      "South & East Asia (n = ",
      sum(data_included_documented_reg$continent_final == "South & East Asia"),
      ")"
    ),
    paste0(
      "Latin & South America (n = ",
      sum(
        data_included_documented_reg$continent_final == "Latin & South America"
      ),
      ")"
    ),
    paste0(
      "North America (n = ",
      sum(data_included_documented_reg$continent_final == "North America"),
      ")"
    )
  ),
  5
